...
import {
  ...
  updateSlide,
  deleteSlide,
} from "../../../services/aboutUsService/cofoundersService";

const CofoundersManagement: React.FC = () => {
  const [cofoundersData, setCofoundersData] =
    useState<ParsedCofoundersSlider | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [showModal, setShowModal] = useState(false);
  const [showSlideModal, setShowSlideModal] = useState(false);
  const [editingCofounders, setEditingCofounders] =
    useState<ParsedCofoundersSlider | null>(null);
  const [editingSlide, setEditingSlide] = useState<CofoundersSlide | null>(
    null
  );
  ...
  const loadCofoundersData = async () => {
    try {
      setLoading(true);
      const data = await getCofoundersSlider();
      setCofoundersData(data);
      setError(null);
    } catch (err) {
      setError(
        err instanceof Error ? err.message : "Failed to load cofounders data"
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleEditClick = () => {
    if (cofoundersData) {
      setEditingCofounders({ ...cofoundersData });
      setShowModal(true);
    }
  };
  ...
  const handleSaveCofounders = async () => {
    if (!editingCofounders) return;
    ...
    try {
      ...
      await updateTitleContent(
        editingCofounders._id,
        editingCofounders.title,
        editingCofounders.content
      );

      await loadCofoundersData();
      setShowModal(false);
      setEditingCofounders(null);
    } catch (err) {
      setError(
        err instanceof Error ? err.message : "Failed to save cofounders data"
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleEditSlideClick = (slide: CofoundersSlide) => {
  ...
  const handleSaveSlide = async () => {
    ...
    try {
      ...
      if (isEditingSlide && editingSlide) {
        ...
        // PUT: send arabic/english/image as form-data
        if (!selectedFile) {
  ...
  const updateCofoundersField = (
    ...
    if (field === "title" || field === "content") {
      setEditingCofounders({
        ...editingCofounders,
        [field]: {
          ...editingCofounders[field as "title" | "content"],
          [language]: value,
        },
      });
    }
  ...
  if (loading && !cofoundersData) {
    return (
      <div className="page-wrapper">
        <div className="content">
          <div className="card">
            <div className="card-body text-center">
              <h5>Loading cofounders data...</h5>
            </div>
          </div>
        </div>
        <CommonFooter />
    ...
    <div className="page-wrapper">
      <div className="content">
        <div className="d-lg-flex align-items-center justify-content-between mb-4">
          <div>
            <h2 className="mb-1">Cofounders Management</h2>
            <p>Manage your website cofounders content and images</p>
          </div>
            ...
            <div className="card-body">
              <div className="row">
                <div className="col-md-6">
                  ...
                  <p>
                    <strong>Title:</strong>{" "}
                    {cofoundersData.title.english || "No title set"}
                  </p>
                </div>
                <div className="col-md-6">
                  ...
                  <p>
                    ...
                    {cofoundersData.title.arabic || "لم يتم تعيين عنوان"}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="card-body">
            <div className="table-responsive">
              <table className="table table-hover">
                ...
                <tbody>
                  {cofoundersData && cofoundersData.slides.length > 0 ? (
                    cofoundersData.slides.map((slide) => (
                      <tr
                        key={slide._id}
      ...
      {showModal && editingCofounders && (
        <div className="modal fade show" style={{ display: "block" }}>
          <div className="modal-dialog modal-dialog-centered modal-lg">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">Edit Cofounders Title & Content</h5>
                <button
                  type="button"
                  className="btn-close"
                  ...
                  <div className="tab-pane active" id="english-content">
                    <div className="mb-3">
                      ...
                      <input
                        ...
                        value={editingCofounders.title?.english || ""}
                        onChange={(e) =>
                          updateCofoundersField(
                            "title",
                            e.target.value,
                            "english"
                          )
                  ...
                  <div className="tab-pane" id="arabic-content">
                    <div className="mb-3">
                      ...
                      <input
                        type="text"
                        className="form-control"
                        value={editingCofounders.title?.arabic || ""}
                        onChange={(e) =>
                          updateCofoundersField(
                            "title",
                            e.target.value,
...
export default CofoundersManagement;
