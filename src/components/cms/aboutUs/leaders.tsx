...
import {
  ...
  updateSlide,
  deleteSlide,
} from "../../../services/aboutUsService/leadersService";

const LeadersManagement: React.FC = () => {
  const [leadersData, setLeadersData] = useState<ParsedLeadersSlider | null>(
    null
  );
  ...
  const [editingLeaders, setEditingLeaders] =
    useState<ParsedLeadersSlider | null>(null);
  const [editingSlide, setEditingSlide] = useState<LeadersSlide | null>(null);
  ...
  const loadLeadersData = async () => {
    try {
      setLoading(true);
      const data = await getLeadersSlider();
      setLeadersData(data);
      setError(null);
    } catch (err) {
      setError(
        err instanceof Error ? err.message : "Failed to load leaders data"
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleSaveLeaders = async () => {
    ...
    try {
      ...
      await updateTitleContent(
        editingLeaders._id,
        editingLeaders.title,
        editingLeaders.content
      );
      ...
      setError(
        err instanceof Error ? err.message : "Failed to save leaders data"
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleEditSlideClick = (slide: LeadersSlide) => {
  ...
  const handleSaveSlide = async () => {
    ...
    try {
      ...
      if (isEditingSlide && editingSlide) {
        ...
        await addSlide(leadersData._id, selectedFile, arabicData, englishData);
      }
  ...
  const updateLeadersField = (
    ...
    if (field === "title" || field === "content") {
      setEditingLeaders({
        ...editingLeaders,
        [field]: {
          ...editingLeaders[field as "title" | "content"],
          [language]: value,
        },
      });
    }
  ...
  if (loading && !leadersData) {
    return (
      <div className="page-wrapper">
        <div className="content">
          <div className="card">
            <div className="card-body text-center">
              <h5>Loading leaders data...</h5>
            </div>
          </div>
        </div>
        <CommonFooter />
    ...
    <div className="page-wrapper">
      <div className="content">
        <div className="d-lg-flex align-items-center justify-content-between mb-4">
          <div>
            <h2 className="mb-1">Leaders Management</h2>
            <p>Manage your website leaders content and images</p>
          </div>
        ...
        {leadersData && (
          <div className="card mb-4">
            <div className="card-header">
              <h5 className="card-title">Leaders Information</h5>
            </div>
            <div className="card-body">
              <div className="row">
                <div className="col-md-6">
                  ...
                  <p>
                    <strong>Title:</strong>{" "}
                    {leadersData.title.english || "No title set"}
                  </p>
                </div>
                <div className="col-md-6">
                  ...
                  <p>
                    ...
                    {leadersData.title.arabic || "لم يتم تعيين عنوان"}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="card-body">
            <div className="table-responsive">
              <table className="table table-hover">
                ...
                <tbody>
                  {leadersData && leadersData.slides.length > 0 ? (
                    leadersData.slides.map((slide) => (
                      <tr
                        key={slide._id}
      ...
      {showModal && editingLeaders && (
        <div className="modal fade show" style={{ display: "block" }}>
          <div className="modal-dialog modal-dialog-centered modal-lg">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">Edit Leaders Title</h5>
                <button
                  type="button"
                  className="btn-close"
                  ...
                  <div className="tab-pane active" id="english-content">
                    <div className="mb-3">
                      ...
                      <input
                        ...
                        value={editingLeaders.title?.english || ""}
                        onChange={(e) =>
                          updateLeadersField("title", e.target.value, "english")
                        }
                      />
                    </div>
                  </div>
                  ...
                  <div className="tab-pane" id="arabic-content">
                    <div className="mb-3">
                      ...
                      <input
                        type="text"
                        className="form-control"
                        value={editingLeaders.title?.arabic || ""}
                        onChange={(e) =>
                          updateLeadersField("title", e.target.value, "arabic")
                        }
                        dir="rtl"
                      />
                    </div>
