...
import { serviceFeaturesService } from "@/services/servicesService/serviceFeaturesService";
import {
  ServiceFeaturesData,
  ServiceFeaturesSlideFormData,
} from "@/types/servicesTypes/serviceFeatures";

const ServiceFeaturesManagement: React.FC = () => {
  const [data, setData] = useState<ServiceFeaturesData | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [showSlideModal, setShowSlideModal] = useState(false);
  const [editingSlide, setEditingSlide] =
    useState<ServiceFeaturesSlideFormData | null>(null);
  ...
  const loadServiceFeaturesData = async () => {
    try {
      setLoading(true);
      const response = await serviceFeaturesService.getServiceFeatures();
      setData(response);
  ...
  // Filter slides based on selected service name
  ...
  const handleEditSlideClick = (slide: ServiceFeaturesSlideFormData) => {
  ...
  const handleSaveSlide = async () => {
    ...
    try {
      ...
      if (isEditingSlide && editingSlide && editingSlide._id) {
        await serviceFeaturesService.updateSlide(
          editingSlide._id,
        ...
        await serviceFeaturesService.addSlide(
          data.slider._id,
          selectedFile,
          {
  ...
  const handleDeleteSlide = async (slideId: string) => {
    ...
    try {
      ...
      await serviceFeaturesService.deleteSlide(slideId);
    ...
    <div className="page-wrapper">
      <div className="content">
        <div className="d-lg-flex align-items-center justify-content-between mb-4">
          <div>
            <h2 className="mb-1">Service Features Management</h2>
            <p>Manage your website service features content and images</p>
          </div>
