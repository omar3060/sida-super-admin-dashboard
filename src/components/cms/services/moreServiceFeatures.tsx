...
import { moreServiceFeaturesService } from "@/services/servicesService/moreServiceFeaturesService";
import {
  ServiceFeaturesData,
  ServiceFeaturesSlide,
} from "@/types/servicesTypes/serviceFeatures";

const MoreServiceFeaturesManagement: React.FC = () => {
  const [data, setData] = useState<ServiceFeaturesData | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [showSlideModal, setShowSlideModal] = useState(false);
  const [editingSlide, setEditingSlide] = useState<ServiceFeaturesSlide | null>(
    null
  );
  const [isEditingSlide, setIsEditingSlide] = useState(false);
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const [selectedFilter, setSelectedFilter] = useState<string>("all");
  const [filteredSlides, setFilteredSlides] = useState<ServiceFeaturesSlide[]>(
    []
  );
  ...
  useEffect(() => {
    loadServiceFeaturesData();
  }, []);

  const loadServiceFeaturesData = async () => {
    try {
      setLoading(true);
      const response = await moreServiceFeaturesService.getServiceFeatures();
      setData(response);
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Failed to load service features data"
      );
    } finally {
      setLoading(false);
    }
  ...
  const getUniqueServiceNames = () => {
    if (!data) return [];
    const serviceNames = data.slider.slides.map((slide) => slide.text);
    return [...new Set(serviceNames)].sort();
  };

  const handleFilterChange = (serviceName: string) => {
    setSelectedFilter(serviceName);
    if (serviceName === "all") {
      setFilteredSlides(data?.slider.slides || []);
    } else {
      const filtered =
        data?.slider.slides.filter((slide) => slide.text === serviceName) || [];
  ...
  const handleEditSlideClick = (slide: ServiceFeaturesSlide) => {
    setEditingSlide({
      arabic: slide.arabic,
  ...
  const handleSaveSlide = async () => {
    ...
    try {
      ...
      if (isEditingSlide && editingSlide) {
        await moreServiceFeaturesService.updateSlide(
          editingSlide._id,
        ...
        await moreServiceFeaturesService.addSlide(
          data.slider._id,
          selectedFile,
          {
  ...
  const handleDeleteSlide = async (slideId: string) => {
    ...
    try {
      ...
      await moreServiceFeaturesService.deleteSlide(slideId);
        ...
        <div className="content">
          <div className="card">
            <div className="card-body text-center">
              <h5>Loading service features data...</h5>
            </div>
          </div>
        </div>
        <CommonFooter />
          ...
          <div className="card">
            <div className="card-body text-center">
              ...
              <button
                ...
                onClick={loadServiceFeaturesData}
              >
                Try Again
              </button>
            </div>
    ...
    <div className="page-wrapper">
      <div className="content">
        <div className="d-lg-flex align-items-center justify-content-between mb-4">
          <div>
            <h2 className="mb-1">More Service Features Management</h2>
            <p>Manage your website more service features content and images</p>
          </div>
            ...
            <li>
              <div className="d-flex align-items-center me-3">
                ...
                >
                  ...
                  {getUniqueServiceNames().map((serviceName) => (
...
export default MoreServiceFeaturesManagement;
