...
const API_URL = "https://super-admin-eta.vercel.app/section/home/features";
...
export const featuresService = {
  getFeatures: async (): Promise<Features> => {
    ...
    try {
      ...
      return await handleApiResponse(response, "fetch features data");
    } catch {
  ...
  updateFeatures: async (
    ...
    try {
      ...
      if (featuresData.arabic) {
        formData.append("arabic", JSON.stringify(featuresData.arabic));
      }

      if (featuresData.english) {
        formData.append("english", JSON.stringify(featuresData.english));
      }
      ...
      return await handleApiResponse(response, "update features");
    } catch {
