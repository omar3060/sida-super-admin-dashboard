...
const API_URL = "https://super-admin-eta.vercel.app/section/home/hero";
...
export const heroService = {
  getHero: async (): Promise<Hero> => {
    try {
      ...
      return await handleApiResponse(response, "fetch hero data");
    } catch (error) {
      throw error;
    }
  },

  updateHero: async (heroData: Partial<Hero>, image?: File): Promise<Hero> => {
    try {
      const formData = new FormData();

      if (heroData.arabic) {
        formData.append("arabic", JSON.stringify(heroData.arabic));
      }

      if (heroData.english) {
        formData.append("english", JSON.stringify(heroData.english));
      }
      ...
      return await handleApiResponse(response, "update hero");
    } catch (error) { 
      throw error;
    }
  },
